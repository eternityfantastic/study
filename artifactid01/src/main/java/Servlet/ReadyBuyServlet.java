...
//        "12-2,13-3,14-5"
        ...
        for (String s : split) {
            ...
            //
            if (goods != null) {
                goods.setBuyNum(Integer.parseInt(str[2]));
                goodsList.add(goods);
            }
        }
        ...
        for (OrderItem orderItem : order.orderItemsList) {
            resp.getWriter().println("<1i>" + orderItem.getGoodsName() + " " + orderItem.getGoodsNum() + " " + orderItem.getGoodsUnit() + " " + orderItem.getGoodsPrice() + "</1i>");
            resp.getWriter().println(" </o1>");
            ...
            resp.getWriter().println("<a href=\" ReadyBuyServlet\">确认</a>");
